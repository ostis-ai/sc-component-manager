...
| `components search`  | `cs`, <br/> `comp search`, <br/>`comp s` | Search a component specification in the knowledge base by author, class or explanation substring. | [--author \<author\>] <br/> [--class \<class\>] <br/> [--key \<key\>] <br/> [--explanation \<"explanation"\>] or [--explanation \<explanation\>] <br/> [--note \<"note"\>] or [--note \<note\>] <br/> [--purpose \<"purpose"\>] or [--purpose \<purpose\>] <br/> [--main-id \<"main identifier"\>] or [--main-id \<main identifier\>]   |
